...
// ------------- Global Variables ---------------- //
var imagesCachingStarted = false;
var result = null;
...
function showErrorImage(lang, type, errorMessage="") {
	...
	var imageDesc = t("loadingSkipped");
...
function preloadImagesIOTD() {
	...
	var images = result.feed.entries.map(entry => {
		const url = new URL(entry.enclosures[0].url);
	...
	navigator.serviceWorker.ready.then(() => {
		...
		for (let i = 0; i < 5; i++) { // 5 images at once
...
function updateAboutMeImage(lang, random = false) {
	...
	.then(
		response => {
			...
			if (error) {
				showErrorImage(lang, "error", `Feednami ${error.code} ${error.message}`);
				adjustScrollDiv();
				return;
			}
...
function updateAboutMeImage2(lang, i) {
	...
	var loadingDivTitle = document.getElementById("loadingDivTitle");
	loadingDivTitle.innerHTML = t("readingImage") + " #" + (i + 1) +" ("+formatBytes(item.enclosures[0].length)+"). ";
