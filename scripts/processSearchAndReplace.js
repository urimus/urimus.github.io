...
function showInformation(lang) {
	...
	xhr.onreadystatechange = function(){
		if (this.readyState==4 && this.status==200) {
			...
			if (lang.localeCompare('rus')==0) infoText="HTML Редактор Версия 1.0. Создано - 22е Янв, 2018, Последнее Изменение - ";
	...
	xhr.open("GET","scripts/php/dir.php?q="+encodeURIComponent("scripts/processSearchAndReplace.js"),true);
	xhr.send();
}
...
;
		...
		window.location.href='html_editor_'+lang+'.html?pattern='+ encodeURIComponent('spec/mo*')+'&i=0';
		return;
	}
}
