var selEncodingPixelPosInEncodings; // global variable
    ...
    if (searchPatt && searchPatt!="") {
		...
		xhr.onreadystatechange = function(){
			if (this.readyState==4 && this.status==200) {
				...
				if (dir.length>0) {
					processReplace(lang, action, dir, 0, replaceWhat, replaceTo, 0, 0);
				} else {
					return false;
				}
			}
...
function processReplace(lang, action, dir, i, replaceWhat, replaceTo, statisticsTimesReplaced, statisticsFilesProcessed) {
		...
		if (this.readyState==4 && this.status==200) {
			...
			if (removeBom(this.responseText).substring(0,20).localeCompare('Unable to open file!')==0) {
				...
				if (document.getElementById("fileName").getAttribute("href").localeCompare(dir[i]['correctDir']+dir[i]['basename'])==0) {
					document.getElementById("dateModified_lbl").innerHTML=formatDate(removeBom(modified), lang);
...
function resizeImage(lang) {
	...
	newFilePath = prompt(prompt1, "icons/movies/");
        ...
        if (this.readyState==4 && this.status==200) {
			if (removeBom(this.responseText).substring(0,20).localeCompare('Unable to open file!')==0) {
				...
				if (document.getElementById("fileName").getAttribute("href").localeCompare(filename)==0) {
					document.getElementById("dateModified_lbl").innerHTML=formatDate(removeBom(this.responseText), lang);
				}
				return;
...
function uploadFile(lang, allFiles, i, totalFiles, newFilePath, newImageWidth) {
	...
	xhr.open("POST", "scripts/php/upload.php?path="+encodeURIComponent(newFilePath)+"&width="+encodeURIComponent(newImageWidth), true);
	xhr.send(dataArray);
...
function showInformation(lang) {
	...
	xhr.onreadystatechange = function(){
		if (this.readyState==4 && this.status==200) {
			...
			if (lang.localeCompare('rus')==0) infoText="HTML Редактор Версия 1.0. Создано - 22nd of Jan, 2018, Последнее Изменение - ";
			if (lang.localeCompare('eng')==0) infoText="HTML Editor Version 1.0. Created At - 22nd of Jan, 2018, Last Modification - ";
			alert(infoText+formatDate(dir_[0]['modified']), lang);
		}
	};
...
function loadAndShowFile(lang, filename, modified, encoding, totalFiles){
	...
	xhr.onreadystatechange = function(){
		if (this.readyState==4 && this.status==200) {
			...
			document.getElementById("dateModified_lbl").innerHTML=formatDate(modified, lang);
			$("#error_message_row").hide();
