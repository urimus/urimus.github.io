...
function replace(lang, action) {
	...
	let confirm2 = window.confirm(t("useScript"));
	if (confirm2) {
		replaceScript(lang);
	} else {
		replacePHP(lang, action);
...
function replaceScript(lang) {
	let lines = document.getElementById("textarea_area").value;

	////////////////////////////////// Script /////////////////////////
	let fileContents = lines.split('\n'); 
	let statisticTimesReplaced = 0;
	let statisticLinesReplaced = "";
	for (let j = 0; j < fileContents.length; j++) { 
		let line = fileContents[j];
		let searchPos = line.indexOf('data-description="');
		if (searchPos==-1) continue;
		let searchPosQ1 = line.indexOf('"', searchPos+1);
		let searchPosQ2 = line.indexOf('"', searchPosQ1+1);
		let searchPosDT = line.substring(0, searchPos).indexOf('data-title');
		if (searchPosDT!=-1) continue;

		let searchPosD = line.indexOf('.', searchPosQ1+1);
		let searchPosA = line.indexOf('<a', searchPosQ1+1);
		if (searchPosD==-1) continue;
		if (searchPosA<searchPosD) continue;

		let titleText = line.substring(searchPosQ1 + 1, searchPosD).trim();
		fileContents[j] = line.substring(0, searchPos) + "data-title=\"" + titleText + "\" data-description=\"" + line.substring(searchPosD+2);
		statisticTimesReplaced++;
		statisticLinesReplaced = statisticLinesReplaced + ", " + j;
	}
	let lines2 = fileContents.join('\n');
	////////////////////////////////// End of Script /////////////////////////
	...
	if (statisticTimesReplaced>0) {
		...
		requestAnimationFrame(() => { // to ensure changes applied
			alert(t("fileModifiedByScript") + statisticLinesReplaced.substring(2));
		});
	} else {
...
function replacePHP(lang, action = "replace") {
	...
	if (textAreaChanged == 1) {
		let fileHref = document.getElementById("fileName").getAttribute("href");
		let confirmContinue = window.confirm(t("file") + "'" + fileHref + "' " + t("notSavedContinue"));
	...
	if (action=="replace") {
		if (replaceWhat=="") {
			if (document.selection != undefined)  { // IE version
				...
				let lines = replaceWhat.split(String.fromCharCode(10));
		...
		let confirmReplace = window.confirm(t("reallyReplace") + replaceWhat + t("to") + replaceTo + "' " + t("inAllDocuments") + getParameterByName('pattern') + "' ?");
	...
	if (action=="addMenu" || action=="removeMenu") {
		let confirmMenu = window.confirm(t("really") + ((action=="addMenu") ? t("add") : t("remove")) + t("menuHeight") + t("inAllDocuments") + getParameterByName('pattern') + "' ?");
	...
	let searchPatt = getParameterByName('pattern'); 
...
function processReplacePHP(lang, action, dir, i, replaceWhat, replaceTo, statisticsTimesReplaced, statisticsFilesProcessed) {
	let statisticsTimesReplaced_o = statisticsTimesReplaced;
	...
	let captionDiv = document.getElementById("caption_div");
	...
	.then(
		response => {
			...
			if ((i + 1) == dir.length) {
				...
				let fullMessage = (dir.length != 1) ? t("filesReplacedSuccessfully") : t("fileReplacedSuccessfully");
				...
				requestAnimationFrame(() => {
					let confirmLog = window.confirm(fullMessage);
					if (confirmLog) {
						let logFile = "scripts/php/logs/";
...
function resizeImage(lang) {
	let locStPar = "HTML_editor";
	let HTMLEditorData = getLocalStorageData(locStPar);
	let newFilePath = (typeof HTMLEditorData["resizeImagePath"] !== "undefined") ? HTMLEditorData["resizeImagePath"] : "images/icons/movies";
	...
	img.onload = function() {
		...
		let newImageWidth = prompt(t("enterImageWidth"), imageWidth);
...
function resizeImages(lang) {
	...
	let locStPar = "HTML_editor";
	let HTMLEditorData = getLocalStorageData(locStPar);
	let newFilePath = (typeof HTMLEditorData["resizeImagesPath"] !== "undefined") ? HTMLEditorData["resizeImagesPath"] : "images/icons/movies";
	...
	let newImageWidth = prompt(t("enterImagesWidth"), "350");
	if (newImageWidth == null) return;
...
function prResizeImage(lang, dir, filePath, fileWidth, i, statisticsFilesProcessed) {
	...
	.then(
		response => {
			...
			if ((i + 1) == dir.length) {
				captionDiv.textContent = t("htmlEditor");
				let fullMessage = (dir.length != 1) ? t("filesResizedSuccessfully") : t("fileResizedSuccessfully");
...
function save(lang, encoding = "UTF-8", filename = document.getElementById("fileName")?.getAttribute("href") ?? "", showMessage = true, message = "") {

	let message1 = message === "" ? t("fileSavedSuccessfully") : message;
...
function saveas(lang, encoding = "UTF-8") {
	...
	let filename_orig_dir=document.getElementById("fileName").getAttribute("href").substring(0,document.getElementById("fileName").getAttribute("href").lastIndexOf("/")+1);
	let newFileName = prompt(t("enterFileName"), document.getElementById("fileName").getAttribute("href"));
	let newFileName_dir=newFileName.substring(0,newFileName.lastIndexOf("/")+1);
	...
	.then(
		response => {
			...
			if (data === 0) {
				...
				save(lang, encoding, newFileName, true, message);
			} else { 
				let confirm = window.confirm(t("file") + newFileName + t("existsOverwrite"));
				if (!confirm) return;
				save(lang, encoding, newFileName, true, t("fileSavedAsSuccessfully"));
			}
		},
		consoleAxiosError
	);
...
function upload(lang) {
	...
	input.onchange = e => { 
		...
		let prompt1, locStPar, HTMLEditorData, newFilePath;
		...
		.then(
			response => {
				...
				if (typeof data === "string" && data=="not logged in") {alert(t("notLoggedIn")); return;}
				if (data === 0) { 
					let createFolder = window.confirm(t("folder")+ newFilePath+ t("notFoundCreate"));
					if (!createFolder) return;
					upload2(lang, allFiles, 0, newFilePath, 1);
				} else { 
					upload2(lang, allFiles, 0, newFilePath, 0);
				}
			},
			consoleAxiosError
		);
...
function upload2(lang, allFiles, i, newFilePath, createFolder) {
	...
	if (filetype.startsWith("image")) {
		isImage=true;
		imagetype=filetype.substring(6);
	}
	...
	if (imagetype!="jpeg") {
		...
		fileReader.onload = function(event) {
			...
			let arr1 = new Uint8Array(arrayBuffer);
			let bytes = [];
...
function upload3(file, filename, isJpg, toJpg, lang, allFiles, i, newFilePath, createFolder) {
	...
	img.onload = function () {
		...
		if ((isJpg || !toJpg) && imageWidth==newImageWidth) {
			upload4(file, filename, lang, allFiles, i, newFilePath, createFolder);
			return;
		}

		let ratio = newImageWidth / imageWidth;
...
function upload4(file, filename, lang, allFiles, i, newFilePath, createFolder) {
	...
	let message = file.type.startsWith("image") ? t("image") : t("file");
	...
	.then(
		response => {
			...
			if (data === 0) {
				...
				let confirm = window.confirm(message + newFilePath + "/" + filename + t("existsOverwrite"));
				if (!confirm) return;
				uploadFile(file, filename, lang, allFiles, i, newFilePath, createFolder);
			}
		},
...
function uploadFile(file, filename, lang, allFiles, i, newFilePath, createFolder) {
	...
	let message = isImage ? t("image") : t("file");
	...
	let confirm = window.confirm(summary);
	if (!confirm) return;
	...
	loadingDiv.style.paddingBottom = "1px"; // to avoid spin shift
...
function del(lang, totalFiles) {
	...
	let filename=document.getElementById("fileName").getAttribute("href");
	let confirm = window.confirm(t("reallyDelete")+" "+filename+"?");
	if (!confirm) return;
	...
	.then(
		response => {
			...
			if (data === 1) {
				alert(t("fileDeleted"));
				let i =(parseInt(getParameterByName('i'))||0);
...
function generateSitemap(lang) {
	...
	let locStPar = "HTML_editor";
	let HTMLEditorData = getLocalStorageData(locStPar);
	let baseUrl1 = (typeof HTMLEditorData["baseUrl"] !== "undefined") ? HTMLEditorData["baseUrl"] : "https://urimus.github.io/";
	...
	.then(
		response => {
			const data = response.data;
			let message = "Sitemap.xml " + t("and") + " all.html " + t("and") + " robots.txt " + t("for") + " '" + data + "'" + t("generatedSuccessfullyShowThem");
			let confirmView = window.confirm(message);
...
function searchPattern(lang) {
	let pattern=document.getElementById('search_pattern').value;
...
function processSearch(lang) {
	...
	if (searchPatt && searchPatt!="") {
		...
		let err1 = t("notFoundFiles") + searchPatt + "'.";
		let i=(parseInt(getParameterByName('i'))||0);
		if (i!=null) {
			axios.get("scripts/php/dir.php", {
				params: {
				...
				response => {
					...
					if (dir.length>0) {
						...
						if (dir[i]!="") {
							...
							let alphabetFilesCount = {};
							let alphabetFilesHighlightedLetter;
							let highlightedFilesCount=1;
							let prevLetterShown="";
							let filesContAlphabetDiv=document.getElementById("files_cont_alphabet_div");
							let filesContDiv=document.getElementById("files_cont_div");
							let j, firstLetter, a;
...
function detectBom(bytes) {
	for (let c = 0; c < bytes.length; c++ ) {
		if (c==5) return null;
		let bytesDetected=detectBomCheckSoFar(bytes.slice(c,c+5));
...
function setBOM(first10bytes) {
	...
	let i=0;
	let bomDetected=detectBom(first10bytes);

	if (bomDetected) {
		bomDetected.pop();
		let bomDiv=document.getElementById("bom_div");
		bomDiv.innerHTML="";
	...
	let bomDiv2=document.getElementById("bom_div2");
	bomDiv2.innerHTML="";
	for (let j=i; j<first10bytes.length; j++) {
		if (j==i) {
...
function setTextAreaChanged(lang, isChanged = true) {

	let changedDiv=document.getElementById("textarea_changed_div");
	if (isChanged) {
		textAreaChanged=1;
	...
	.then(
		response => {
			...
			if (data === 0){
				let locStPar="HTML_editor";
				let HTMLEditorData=getLocalStorageData(locStPar);
				let username1 = (typeof HTMLEditorData["username"] !== "undefined") ? HTMLEditorData["username"] : "";
				let username = prompt(t("enterUsername"), username1);
