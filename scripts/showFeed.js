...
function processShowFeedData(type, source, lang, result) {
	...
	showEntry(type, source, lang, items, 0, items[0].media.url, tableMainRow);
}


function showEntry(type, source, lang, items, i, imagesrc, tableMainRow) {
	var entry = items[i];
	if (source=="bbc") {
		...
		localforage.getItem(entry.media.url, function (err, value) {
			if (value == null) {
				...
				if (source=="nasa" && imagesrc!="images/icons/feed/nasa_news.png") {
					imagesrc="https://api.codetabs.com/v1/proxy/?quest="+imagesrc;
				}
				showEntry2(type, source, lang, items, i, imagesrc, tableMainRow, 1);
			} else {
				blobLink= window.URL.createObjectURL(value);
				showEntry2(type, source, lang, items, i, blobLink, tableMainRow, 0);
			}
		});
	}
}

function showEntry2(type, source, lang, items, i, imagesrc, tableMainRow, toSave) {
...
// console.log(entry);
	...
	if (i+1 < totalEntries) showEntry(type, source, lang, items, i+1, imagesrc, tableMainRow);
}
...
//console.log("data="+data);
			...
			if (matchPos!=-1) { // image
				...
				// decode html
				mediaURL=$("<textarea/>").html(mediaURL).text();
				if (mediaURL.substr(0,4)!="http") {
					matchPos=result.entries[i].link.indexOf("/",8);
					serverName=result.entries[i].link.substr(0,matchPos);
					mediaURL=serverName+mediaURL;
				}
...
//console.log(data);
			...
			searchStrArr.push("property=\"twitter:description\" content=\"");
			searchStrArr.push("name=\"og:description\" content=\"");
			searchStrArr.push("property=\"og:description\" content=\"");
			
			for (j=0; j<searchStrArr.length;j++) {
				searchStr=searchStrArr[j];
				matchPos=data.indexOf(searchStr);
				if (matchPos!=-1) break;
			}
			description="";
