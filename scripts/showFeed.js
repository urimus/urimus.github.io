...
function creatorUrlText (name, url) {
	var a = document.createElement('a');
...
function showRecordsNum(type, recordsNum, lang, newCaption) {
	...
	document.getElementById('feed_'+type).setAttribute("title", recordsNum + " " + t("record", { count: recordsNum }));
...
function preloadImage(type, source, lang, result) {
	...
	var newUrl = entry.media.url;
	if (newUrl.substr(newUrl.length - 12) != "no_image.png") {
	...
	isImageCached(newUrl).then(function (cached) {
...
//						ifrm.src="https://cdn.theoplayer.com/demos/iframe/theoplayer.html?autoplay=false&muted=false&preload=none&src="+entry.video;
	...
	if (typeof entry.creator !== "undefined" && entry.creator.length > 0 && typeof entry.creator[0] !== "undefined") {
		...
		if (entry.creator.length > 1) {
			Div.innerHTML = "<b>" + t("creators") + ":&nbsp;</b>";
			if (entry.creatorUrl && entry.creatorUrl[0]) {
				Div.innerHTML += creatorUrlText(entry.creator[0], entry.creatorUrl[0]);
			} else {
			...
			for (var j = 1; j < entry.creator.length; j++) {
				if (entry.creatorUrl && entry.creatorUrl[j]) {
					Div.innerHTML += ",&nbsp;" + creatorUrlText(entry.creator[j], entry.creatorUrl[j]);
				} else {
					Div.innerHTML += ",&nbsp;" + entry.creator[j];
				}
				if (typeof entry.creatorEmail !== "undefined" && typeof entry.creatorEmail[j] !== "undefined") {
					Div.innerHTML += '&nbsp;' + mailToIconText(entry.creatorEmail[j]);
				}
			}
		} else {
			if (entry.creatorUrl && entry.creatorUrl[0]) {
				Div.innerHTML = "<b>" + t("creator") + ":&nbsp;</b>" + creatorUrlText(entry.creator[0], entry.creatorUrl[0]);
			} else {
				Div.innerHTML = "<b>" + t("creator") + ":&nbsp;</b>" + entry.creator[0];
			}
			if (typeof entry.creatorEmail !== "undefined" && typeof entry.creatorEmail[0] !== "undefined") {
				Div.innerHTML += '&nbsp;' + mailToIconText(entry.creatorEmail[0]);
			}
	...
	if (typeof entry.category !== "undefined" && entry.category.length > 0 && typeof entry.category[0] !== "undefined") {
		...
		if (entry.category.length > 1) {
			Div.innerHTML = "<b>" + t("categories") + ":&nbsp;</b>" + entry.category.join(", ");
		} else {
...
function showFeednamiError(error, feedURL, lang) {
...
function loadFeednami(type, source, lang, feedURL, loadAttempt = 1) {
	...
	.then(
		response => {
			...
			adjustFeedScrollDiv();
			if (result.error) {
				showFeednamiError("Feednami " + result.error.code + " " + result.error.message, feedURL, lang);
				return;
			}
		...
		error => {
			...
			showFeednamiError(error.message, feedURL, lang);
...
function showFeed(type, source, lang) {
	...
	var loadingSpanTitleSub = document.createElement("span");
	loadingSpanTitleSub.id = "loadingSpanTitleSub";
	loadingSpanTitleSub.innerHTML= t("readingNewsFeed") + " ";
	loadingSpanTitle.appendChild(loadingSpanTitleSub);
	loadingSpanTitle.appendChild(feedIcon(feedURL, lang));
	...
	for (var c = 0; c < items.length; c++) {
		...
		if (source == "artemis") {
			if (entry.imagePath!= null) {
				...
				result.entries[i].media.comment = t("imageAbsent");
			}
		...
		if (source == "space.com") {
			if (entry.image != null && entry.image.url != null) {
				...
				result.entries[i].media.comment += "<br>&bull;&nbsp;" + t("credit") + ": ";
		...
		if (source == "artemis" || source == "cbs" || (source == "nasa" && type != "image") || source == "yonhap" || source == "yahoo") { 
			if (typeof locStUpdateData[result.entries[i].link] !== "undefined") {
				...
				if (typeof locStUpdateData[result.entries[i].link].creatorUrl!== "undefined") {
					result.entries[i].creatorUrl = [];
					for (var j = 0; j < locStUpdateData[result.entries[i].link].creatorUrl.length; j++) {
						if (locStUpdateData[result.entries[i].link].creatorUrl[j]) {
							result.entries[i].creatorUrl[j] = locStUpdateData[result.entries[i].link].creatorUrl[j];
						}
					}
				}
	...
	.then(
		response => {
			...
			const getScripts = (doc, search) => {
				...
				return [...scripts].map(p => p.textContent);
			};

			mediaURL = getMeta(doc, 'meta[property="og:image"]');
