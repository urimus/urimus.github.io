...
function showRecordsNum(type, recordsNum, lang, newCaption) {
	...
	if (recordsNum==0) {
		document.getElementById('feed_'+type).onmouseleave = function(){mouseOut(type, encodeURIComponent(getParameterByName('type')), 'black');}
...
function showFeedTitle(type, source, lang, result) {
	var textRssFeed, textOutputRecords;
	if (lang == "rus") {
	...
	preloadImg.onload = function () {
		...
		if (source=="nasa" && type=="image") {
			...
			cell1.innerHTML=cell1.innerHTML+' '+textPre+' Brian Dunbar&nbsp;<a href="mailto:brian.dunbar@nasa.gov" class="standardb_red" target="_blank"><img valign="middle" title="brian.dunbar@nasa.gov" class="thumbnail_image_red_png" src="images/icons/feed/mailto_red.svg" height="27"></a>';
		}
	...
	if (typeof entry.creator !== "undefined" && entry.creator.length > 0 && typeof entry.creator[0] !== "undefined") {
		...
		if (entry.creator.length > 1) {
			...
			if (typeof entry.creatorEmail !== "undefined" && typeof entry.creatorEmail[0] !== "undefined") {
				Div.innerHTML += '&nbsp;<a href="mailto:' + entry.creatorEmail[0] + '" class="standardb_red" target="_blank"><img valign="middle" title="' + entry.creatorEmail[0] + '" class="thumbnail_image_red_png" src="images/icons/feed/mailto_red.svg" height="27"></a>';
			}
			for (var j = 1; j < entry.creator.length; j++) {
				...
				if (typeof entry.creatorEmail !== "undefined" && typeof entry.creatorEmail[j] !== "undefined") {
					Div.innerHTML += '&nbsp;<a href="mailto:' + entry.creatorEmail[j] + '" class="standardb_red" target="_blank"><img valign="middle" title="' + entry.creatorEmail[j] + '" class="thumbnail_image_red_png" src="images/icons/feed/mailto_red.svg" height="27"></a>';
				}
			}
		} else {
...
function generateTabs(type, source, lang) {
	...
	for (var i=0; i<keys.length; i++) {
		...
		Div.setAttribute('onmouseleave', "mouseOut('"+keys[i]+"', '"+type+"'); if(typeof this.dataset.fullTitle!=='undefined') this.innerHTML=this.dataset.shortTitle;");
	...
	if (lang=="eng" || lang=="lat") {
		...
		Img.setAttribute('title', "Tip: Use Navigation Keys - <kbd>&rlarr;</kbd>, <kbd>Home</kbd>, <kbd>End</kbd>, <kbd>Page Up</kbd>, <kbd>Page Down</kbd> and <kbd>Shift</kbd> for Contents Scroll");
	}
	if (lang=="rus") {
