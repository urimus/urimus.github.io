...
function showFeedTitle(type, source, lang, result) {
	...
	preloadImg.onload = function () {
		...
		if (source=="nasa" && type=="image") {
			cell1.innerHTML+=' '+t("by")+' Brian Dunbar&nbsp;' + mailToText("brian.dunbar@nasa.gov");
		}
...
function showFeed(type, source, lang) {
	...
	loadingSpanTitle.innerHTML= t("readingNewsFeed") + " " + feedIconText(feedURL, lang);
...
function update(i, source, type, result, lang, updateAttempt = 1) {
	...
	$.ajax({
		type: "GET",
		url: "https://proxy3.wasmer.app?url=" + encodeURIComponent(result.entries[i].link),
		cache: false,
		dataType: "text",
