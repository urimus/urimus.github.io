...
$(function() {
	let r = 4;
	let lineColor = "#ff8a00";
	let currentTooltipTarget = null;
	let suppressTooltipOpen = false;
	let activeTooltips = new Set();
	let updateLoopRunning = false;
	let scrollDiv = document.getElementById('scrollDiv');
	let lastViewportScale = window.visualViewport ? window.visualViewport.scale : 1;
	...
	function drawLine(tooltipEl, targetRect) {
		...
		let tooltipRect = getBoundingClientRectNoSnap(tooltipEl);
		...
		if (!tooltipEl._boundingRect) {
			if (scrollDiv && scrollDiv.contains(tooltipEl._targetEl)) {
				let scrollRect = getBoundingClientRectNoSnap(scrollDiv);
		...
		let distances = { top: Math.abs(tooltipRect.top - targetY), bottom: Math.abs(tooltipRect.bottom - targetY) };
		let minSide = distances.top < distances.bottom ? "top" : "bottom";

		let tooltipX, tooltipY, d, startX, endX;
		...
		let length = Math.hypot(tooltipX - targetX, tooltipY - targetY);
	...
	function positionTooltip(tooltipEl) {
		if (!tooltipEl || !tooltipEl._targetEl) return;
		let insideScroll = scrollDiv && scrollDiv.contains(tooltipEl._targetEl);
		$(tooltipEl).position({
			my: "center bottom",
			at: `center top-${r}`,
			of: tooltipEl._targetEl,
			within: insideScroll ? scrollDiv : svgEl,
			collision: "fit"
		});
	}
	...
	function setTooltipMaxWidth(tooltipEl) {
		...
		let scale = isOpera ? lastViewportScale : 1;
	...
	function supressTooltips() {
		suppressTooltipOpen = true;
		activeTooltips.forEach(tooltipEl => removeTooltip(tooltipEl));
	}
	window.addEventListener("beforeunload", supressTooltips);
	window.addEventListener("pagehide", supressTooltips);
	...
	window.addEventListener("focus", () => {
		...
		if (activeElement?.hasAttribute('title')) {
...
/* alternative version
	window.addEventListener("focus", () => document.activeElement?.closest('a')?.blur());
*/
});
